...
def get_data():
	...
	for img in os.listdir(os.path.join(train_path,'busy')):
		train_imgs_busy.append([os.path.join(train_path,'busy',img),[1,0,0]])
	train_imgs_clear=[]
	for img in os.listdir(os.path.join(train_path,'clear')):
		train_imgs_clear.append([os.path.join(train_path,'clear',img),[0,1,0]])
	train_imgs_semibusy=[]
	for img in os.listdir(os.path.join(train_path,'semi-busy')):
		train_imgs_semibusy.append([os.path.join(train_path,'semi-busy',img),[0,0,1]])

	train_imgs=train_imgs_semibusy+train_imgs_clear+train_imgs_busy

	val_imgs_busy=[]
	for img in os.listdir(os.path.join(val_path,'busy')):
		val_imgs_busy.append([os.path.join(val_path,'busy',img),[1,0,0]])
	val_imgs_clear=[]
	for img in os.listdir(os.path.join(val_path,'clear')):
		val_imgs_clear.append([os.path.join(val_path,'clear',img),[0,1,0]])
	val_imgs_semibusy=[]
	for img in os.listdir(os.path.join(val_path,'semi-busy')):
		val_imgs_semibusy.append([os.path.join(val_path,'semi-busy',img),[0,0,1]])
	val_imgs=val_imgs_semibusy+val_imgs_clear+val_imgs_busy
...
def data_loader(img_list):
	...
	for img_addr,label in img_list:
		try:
			img=cv2.resize(cv2.imread(img_addr),(256,256))[:,:,2]
		except:
			continue
