...
def get_data():
	...
	train_imgs_rainy=[]
	for img in os.listdir(os.path.join(train_path,'rainy')):
		train_imgs_rainy.append([os.path.join(train_path,'rainy',img),[1,0,0]])
	train_imgs_cloudy=[]
	for img in os.listdir(os.path.join(train_path,'cloudy')):
		train_imgs_cloudy.append([os.path.join(train_path,'cloudy',img),[0,1,0]])
	train_imgs_sunny=[]
	for img in os.listdir(os.path.join(train_path,'sunny')):
		train_imgs_sunny.append([os.path.join(train_path,'sunny',img),[0,0,1]])

	train_imgs=train_imgs_rainy+train_imgs_cloudy+train_imgs_sunny

	val_imgs_rainy=[]
	for img in os.listdir(os.path.join(val_path,'rainy')):
		val_imgs_rainy.append([os.path.join(val_path,'rainy',img),[1,0,0]])
	val_imgs_cloudy=[]
	for img in os.listdir(os.path.join(val_path,'cloudy')):
		val_imgs_cloudy.append([os.path.join(val_path,'cloudy',img),[0,1,0]])
	val_imgs_sunny=[]
	for img in os.listdir(os.path.join(val_path,'sunny')):
		val_imgs_sunny.append([os.path.join(val_path,'sunny',img),[0,0,1]])
	val_imgs=val_imgs_rainy+val_imgs_cloudy+val_imgs_sunny
...
def placeholders(img_size_1,img_size_2,img_channel,label_cnt):
	with tf.variable_scope('input'):
		X=tf.placeholder(shape=[None,img_size_1,img_size_2,img_channel],dtype=tf.float32,name='image')
